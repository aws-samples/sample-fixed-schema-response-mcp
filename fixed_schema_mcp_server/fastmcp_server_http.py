#!/usr/bin/env python3
"""
Fixed Schema MCP Server using FastMCP with HTTP transport for deployment.

This version is designed for deployment to ECS/ALB and uses HTTP transport
...
mcp = FastMCP("fixed-schema", transport="http")
...
def generate_mock_response(prompt: str, schema_name: str) -> Dict[str, Any]:
    ...
    if schema_name == "product_info":
        query = prompt.split("about ")[-1].strip() if "about " in prompt else "Sample Product"
        return {
            "name": query,
        ...
        query = prompt.split("about ")[-1].strip() if "about " in prompt else "Sample Person"
        return {
            "name": query,
            "age": 30,
        ...
        query = prompt.split("for ")[-1].strip() if "for " in prompt else "sample endpoint"
        return {
        ...
        query = prompt.split("for ")[-1].strip() if "for " in prompt else "sample issue"
        return {
        ...
        query = prompt.split("about ")[-1].strip() if "about " in prompt else "sample topic"
        return {
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Fixed Schema MCP Server (HTTP) on {host}:{port}")
